...
class MatchGame extends Component {
  state = {
    ...
    randomImgUrl: imagesList[0].imageUrl,
    isGameOver: false,
  }

  componentDidMount() {
    this.timerInterval = setInterval(this.timerValue, 1000)
  }

  timerValue = () => {
    const {countDowm} = this.state
    if (countDowm !== 0) {
      this.setState(prev => ({countDowm: prev.countDowm - 1}))
    } else {
  ...
  renderThumbNailImages = () => {
    const {activeId} = this.state
    const filtered = imagesList.filter(each => each.category === activeId)

    return (
      <ul className="thumbnail-unordred">
        {filtered.map(each => (
          <ThumbnailItem
            key={each.id}
            thumbDetails={each}
            thumbnaiImageClicked={this.thumbnaiImageClicked}
          />
        ))}
      </ul>
    )
  ...
  renderScoreCard = () => {
    ...
    return <ScoreCard score={score} playAgain={this.playAgain} />
  }

  render() {
    const {countDowm, score, randomImgUrl, isGameOver} = this.state
    return (
      <div className="app">
        <Header countDown={countDowm} score={score} />
        {isGameOver ? (
